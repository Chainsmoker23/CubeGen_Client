...
interface AwsArchitectureCanvasProps {
  data: DiagramData;
...
// ====================================================================================
// --- AWS-Specific Canvas Component ---
// ====================================================================================

const AwsArchitectureCanvas: React.FC<AwsArchitectureCanvasProps> = ({ 
  ...
  // AWS-specific color assignment for containers (VPCs, Subnets, Availability Zones)
  const awsContainerColors = useMemo(() => {
    const colors = [
      'var(--color-aws-vpc-1)', 
      'var(--color-aws-vpc-2)', 
      'var(--color-aws-vpc-3)',
    ...
    const awsContainers = data.containers?.filter(c => 
      c.type === 'vpc' || c.type === 'subnet' || c.type === 'availability-zone'
    ).sort((a, b) => a.x - b.x) || []; // Sort by x position for consistent color assignment
  ...
  useEffect(() => {
    ...
    const fitToScreen = () => {
      const contentGroup = svg.select<SVGGElement>('#aws-diagram-content').node();
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative bg-[var(--color-canvas-bg)] rounded-b-2xl">
      <svg ref={forwardedRef} className="w-full h-full absolute inset-0" style={{ cursor: getCursor() }}>
        ...
        <g id="aws-diagram-content" transform={viewTransform.toString()}>
          {/* AWS Containers Layer - with specific styling for VPC, Subnet, AZ */}
          <g>
            {data.containers?.map(container => (
              <AwsArchitectureContainer
                key={container.id}
                container={container}
                data={data}
                ...
                fillColor={container.color || awsContainerColors.get(container.id) || 'var(--color-aws-container-default)'}
                interactionMode={interactionMode}
...
// ====================================================================================
// --- AWS-Specific Shape Renderer Component ---
// ====================================================================================

const AwsShapeRenderer: React.FC<{
  node: ArchNode;
  commonProps: any;
}> = ({ node, commonProps }) => {
...
// ====================================================================================
// --- AWS-Specific Draggable Container Component ---
// ====================================================================================
...
const AwsArchitectureContainer: React.FC<{
  container: Container;
  isSelected: boolean;
  ...
  // AWS-specific styling for different container types
  const getStrokeDasharray = () => {
    switch(container.type) {
      ...
      case 'vpc':
      default:
        return 'none'; // Solid border for VPCs
    }
  };
...
// ====================================================================================
// --- AWS-Specific Draggable Node Component ---
// ====================================================================================

const AwsArchitectureNode: React.FC<{
  node: ArchNode;
  isSelected: boolean;
  ...
  const nodeBody = <AwsShapeRenderer node={node} commonProps={commonProps} />;
...
export default AwsArchitectureCanvas;
